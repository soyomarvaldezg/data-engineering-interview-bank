...
.select(
split(col("value"), r"\s+").alias("date"),
split(col("value"), r"\s+").alias("time"),​
split(col("value"), r"\s+").alias("level"),​
concat_ws(" ", split(col("value"), r"\s+")[3:]).alias("message")
)
...
- [RDD vs DataFrame - Databricks Docs](https://docs.databricks.com/en/spark/latest/rdd-dataframe-dataset.html)
