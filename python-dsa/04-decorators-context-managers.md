...
print(f"{func.name} took {elapsed:.4f}s")
return result
return wrapper
...
logging.error(f"{func.name} raised {type(e).name}: {e}")
raise
return wrapper
...
# conn is the return value from enter

Automatically disconnects (calls exit)
